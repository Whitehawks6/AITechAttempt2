...
export default function TimelineEmbed() {
  ...
  useEffect(() => {
    if (!data) return;
    const tick = () => { if (!isPaused) setCurrent(c => (c + 1) % data.events.length); };
    autoRef.current = setInterval(tick, 8000);
    return () => { if (autoRef.current) clearInterval(autoRef.current); };
  }, [data, isPaused]);

  if (!data) return <div className="py-32 text-center text-cyan text-2xl">Loading…</div>;
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="flex flex-col justify-center p-8 md:w-1/2 md:p-12">
            <p className="mb-3 text-sm font-mono text-gold">
              {event.start_date.year}{event.start_date.month && ` • ${event.start_date.month}${event.start_date.day ? `/${event.start_date.day}` : ''}`}
            </p>
            ...
            {years.map(year => {
              ...
              return (
                <div key={year} className="relative flex flex-col items-center min-w-16">
                  ...
                  <div className={`z-10 h-3 w-3 rounded-full border-2 transition-all ${active ? 'border-cyan bg-cyan scale-150' : 'border-gray-600 bg-gray-500'}`} />
                  <p className={`mt-2 text-xs ${active ? 'font-bold text-cyan' : 'text-gray-400'}`}>{year}</p>
                </div>
              );
            })}
          </div>
          <button onClick={() => timelineRef.current?.scrollBy({ left: -300, behavior: 'smooth' })} className="absolute left-0 top-1/2 -translate-y-1/2 rounded-full bg-cyan/20 p-2 hover:bg-cyan/40">
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button onClick={() => timelineRef.current?.scrollBy({ left: 300, behavior: 'smooth' })} className="absolute right-0 top-1/2 -translate-y-1/2 rounded-full bg-cyan/20 p-2 hover:bg-cyan/40">
            <ChevronRight className="h-5 w-5" />
          </button>
        </div>
      </div>

      <p className="mt-6 text-center text-sm text-gray-400">Event {current + 1} / {data.events.length}</p>
    </div>
  );
}
